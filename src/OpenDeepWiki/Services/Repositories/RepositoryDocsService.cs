...
public class RepositoryDocsService(IContext context, IGitPlatformService gitPlatformService)
{
    ...
    public async Task<IResult> ExportAsync(string owner, string repo, [FromQuery] string? branch = null, [FromQuery] string? lang = null)
    {
        ...
        var rootDirectories = catalogs
            ...
            .Select(c => new { DisplayName = !string.IsNullOrEmpty(c.Slug) ? c.Slug : GetDisplayName(c.Title, c.Path), c.Title })
        ...
        using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
        {
            ...
            // 添加 index.md
            var indexContent = GenerateIndexMarkdown(repository.RepoName, branchEntity.BranchName, rootDirectories.Select(d => (d.DisplayName, d.Title)).ToList());
        ...
        foreach (var catalog in currentLevelItems)
        {
            ...
            if (children.Count > 0)
            {
                ...
                var siblingDirs = children
                    ...
                    .Where(p => !string.IsNullOrEmpty(p.DisplayName))
                    .ToList();
                var indexContent = GenerateDirectoryIndexMarkdown(catalog.Title, siblingDirs.Select(d => (d.DisplayName, d.Title)).ToList());
    ...
    /// 生成 VitePress 索引页面
    /// </summary>
    private static string GenerateIndexMarkdown(string repoName, string branchName, List<(string displayName, string title)> rootDirectories)
    {
        ...
        foreach (var (displayName, title) in rootDirectories)
        {
            sb.AppendLine($"- [{title}]({displayName}/index.md)");
        }
    ...
    /// </summary>
    private static string GenerateDirectoryIndexMarkdown(string sectionTitle, List<(string displayName, string title)> subDirectories)
    {
        var sb = new StringBuilder();
        ...
        if (subDirectories.Count > 0)
        {
            sb.AppendLine("### 目录");
            sb.AppendLine();
            foreach (var (displayName, title) in subDirectories)
            {
                sb.AppendLine($"- [{title}]({displayName}.md)");
            }
        }
