...
public sealed class RspressDocsExporter : IRspressDocsExporter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var bl in languages)
        {
            ...
            var repoIndexRel = $"{_mapper.MapLanguage(bl.LanguageCode)}/{repoSlug}/index.md";

            foreach (var page in site.Pages)
            {
                // repo 级 index.md：共享 docPath 的多仓（如 components/server = Server.Source + Server）
                // 最后写入者胜出会互相覆盖，磁盘上已是其它仓的索引时保留现有文件
                if (string.Equals(page.RelativePath, repoIndexRel, StringComparison.Ordinal) &&
                    ShouldPreserveExistingIndex(writer.ResolvePath(page.RelativePath), repo.RepoName))
                {
                    _logger.LogInformation(
                        "跳过 {Path}：磁盘上已有其它仓库的索引页（共享 docPath 不互相覆盖）。", page.RelativePath);
                    continue;
                }
    ...
    /// <summary>
    /// 判断是否保留磁盘上已有的 repo 级 index.md：其 frontmatter
    /// <c>autoGenerated.source.repo</c> 属于其它仓库时保留（共享 docPath 不互相覆盖）；
    /// 文件不存在、无溯源信息或同仓重导则返回 false（照常覆盖）。
    /// </summary>
    private bool ShouldPreserveExistingIndex(string fullPath, string repoName)
    {
        ...
        {
            var inSource = false;
            foreach (var line in File.ReadLines(fullPath).Take(15))
            {
                if (!inSource)
                {
                    if (line.Trim() == "source:")
                ...
                {
                    var existingRepo = trimmed["repo:".Length..].Trim();
                    return existingRepo != repoName;
                }

                if (trimmed.Length > 0)
                {
                    break; // source 块内下一行不是 repo:，视为无溯源
                }
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "读取现有 index.md 溯源失败，按覆盖处理。Path: {Path}", fullPath);
        }
