...
/// <summary>页面 frontmatter 的 autoGenerated.source 溯源信息（gfx-config/catalog-template.json 规范）。</summary>
public record RspressPageMeta(string Repo, string? Commit = null, string? Upstream = null);
...
public sealed class RspressPathMapper
{
    ...
    /// <param name="pageMeta">溯源信息，非空时为每个页面注入 frontmatter（title/order/autoGenerated）。</param>
    public RspressSite Map(
        IEnumerable<DocCatalog> rootCatalogs,
        string repoSlug,
        string languageCode,
        string repoTitle,
        RspressPageMeta? pageMeta = null)
    {
        ...
        {
            var (slug, isDir) = ProcessCatalog(catalog, repoDir, usedRootSlugs, site, pageMeta, rootOrder);
            rootEntries.Add((slug, isDir, catalog.Title));
            rootOrder++;
        }

        // repo 目录 index.md：若有内容用内容，否则生成子链接列表
        WriteRepoIndex(repoDir, repoTitle, rootEntries, roots, site, pageMeta);
        // repo 目录 _meta.json
        ...
        int order = 1)
    {
        ...
        if (hasChildren)
        {
            ...
            foreach (var child in catalog.Children.OrderBy(c => c.Order)
                                                .ThenBy(c => c.Title, StringComparer.Ordinal))
            {
                var (childSlug, childIsDir) = ProcessCatalog(child, dir, childUsed, site, pageMeta, childOrder);
                childEntries.Add((childSlug, childIsDir, child.Title));
                childOrder++;
            }

            // 目录 index.md
            WriteDirIndex(dir, catalog, childEntries, site, pageMeta, order);
            // 目录 _meta.json
        ...
        else
        {
            ...
            site.Add(new RspressPage(relPath, WithFrontmatter(catalog.Title, order, pageMeta, content!)));
        ...
        RspressPageMeta? pageMeta)
    {
        ...
        site.Add(new RspressPage($"{repoDir}/index.md", WithFrontmatter(repoTitle, 0, pageMeta, content)));
    }
        ...
        int order)
    {
        ...
        site.Add(new RspressPage($"{dir}/index.md", WithFrontmatter(catalog.Title, order, pageMeta, content)));
    }

    /// <summary>
    /// 生成 _meta.json：["leaf", {"type":"dir","name":"x","label":"Y"}]。
    /// <para>刻意不列 "index"：Rspress 的 auto-nav-sidebar 在 _meta.json 未列 index 时，
    /// 会把目录条目自动关联到目录下的 index.md（点击目录即首页），且不会出现重复的同名子章节。</para>
    /// </summary>
    ...
    /// <summary>
    /// 注入 gfx-config 规范的 frontmatter（title/order/autoGenerated.source）。
    /// 内容自带 frontmatter（以 "---" 开头）时不注入，避免双 YAML 块。
    /// </summary>
    private static string WithFrontmatter(string title, int order, RspressPageMeta? meta, string content)
    {
        if (meta == null || content.StartsWith("---", StringComparison.Ordinal))
        {
            return content;
        }
        ...
        var sb = new StringBuilder();
        sb.AppendLine("---");
        // 双引号包 title 防 YAML 特殊字符（冒号等）；标题内双引号罕见，替换为单引号兜底
        sb.Append("title: \"").Append(title.Replace("\"", "'")).AppendLine("\"");
        sb.AppendLine(CultureInfo.InvariantCulture, $"order: {order}");
        sb.AppendLine("autoGenerated:");
        sb.AppendLine("  source:");
        sb.AppendLine(CultureInfo.InvariantCulture, $"    repo: {meta.Repo}");
        if (!string.IsNullOrEmpty(meta.Commit))
        {
            sb.AppendLine(CultureInfo.InvariantCulture, $"    commit: {meta.Commit}");
        }
        sb.AppendLine("    generator: OpenDeepWiki");
        if (!string.IsNullOrEmpty(meta.Upstream))
        {
            sb.AppendLine(CultureInfo.InvariantCulture, $"    upstream: {meta.Upstream}");
        }
        sb.AppendLine("---");
        sb.Append(content);
        return sb.ToString();
